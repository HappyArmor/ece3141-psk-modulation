...
for idx = 1:length(M_list)
    ...
    plot(axWave, t_total, s_total, lineStyles{idx}, ...
        'LineWidth', 1.2, ...
        'DisplayName', [num2str(M) '-PSK']);
    % display random bits used in waveform
    if idx==1
        shownBits = bits(1:numPlotSymbols*log2(max(M_list)));
        shownBitsText = sprintf('%d', shownBits);
        
        text(axWave, mean(xlim(axWave)), max(ylim(axWave))*1.05, ...
            ['Random bits: ' shownBitsText], ...
